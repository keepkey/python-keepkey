...
class CalldataCompiler(object):
    ...
    def compile(self):
        ...
        for field, steps in fields:
            ...
            displays.append((4, string_index[field["label"]], formatter_index, ABSENT))
        displays.append((10, ABSENT, ABSENT, ABSENT))

        if len(paths) > 64 or len(formatters) > 64 or len(displays) > 64:
        ...
        resource = [_u16(len(strings)), _u16(len(flat)), _u16(len(paths)),
                    _u16(len(literals)), _u16(0), _u16(len(formatters)),
                    _u16(len(displays)), _u16(len(bindings)),
                    bytes([max_depth, 0, 0, 0]),
